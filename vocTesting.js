...
function scoreCounting() {
  ...
  let falseCounts = arrays.reduce(
    (total, arr) => total + countFalseValues(arr),
    0
  );
...
function checkAnswer() {
  ...
  for (let i = 1; i <= numOfQ; i++) {
    ...
    if (
      checkedVoc[i - 1] !== false &&
      checkedYomigana[i - 1] !== false &&
      checkedMeaning[i - 1] !== false
    ) {
...
function showIncorrectLine(
  ...
  for (let j = 0; j < 3; j++) {
    ...
    if (j === 0) {
      cell.textContent =
        vocTOrF !== false
          ? tableData[randomIndices[i - 1]].voc
          : vocInput.value;
      if (vocTOrF === false) {
      ...
      cell.textContent =
        yomiganaTOrF !== false
          ? tableData[randomIndices[i - 1]].yomigana
          : yomiganaInput
          ? yomiganaInput.value
          : "";
      if (yomiganaTOrF === false) {
        cell.textContent = yomiganaInput.value
          ? yomiganaInput.value
          : "________";
        cell.style.color = "red";
      }
      ...
      cell.textContent =
        meaningTOrF !== false
          ? tableData[randomIndices[i - 1]].meaning
          : meaningInput
          ? meaningInput.value
          : "";
